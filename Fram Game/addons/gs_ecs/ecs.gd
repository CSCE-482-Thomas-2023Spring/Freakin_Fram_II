...
func add_component(component : Component):
	...
	Logger.fine("%s" % [component.get_property_list()])
	
	if (component.has_method('get_name')):
		_id = component.get_name().to_lower()
		
	if has_component(_id):
		Logger.warn("- component %s was already registered -- skipping" % [_id])
		return _id
				
	component_entities[_id] = {}
	Logger.debug("- new component %s was registered" % [_id])
...
#		add_component(component)
	...
	var _id = add_component(component)
...
func _init() -> void:
	...
	print("Copyright 2018-2022, SpockerDotNet LLC")
